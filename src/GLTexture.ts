...
export class GLTexture {
    ...
    public upload(source: HTMLImageElement | ImageData | HTMLVideoElement) {
        ...
        const newWidth = source.width;
        const newHeight = source.height;
    ...
    public setMinFilter(filter: MIN_FILTER): void {
        if (
            !this.mipmap &&
            (filter === MIN_FILTER.LINEAR_MIPMAP_LINEAR ||
    ...
    public enableMipmap() {
        if (
            GLTexture.isPowerOf2(this._width) &&
    ...
    public static fromSource(
        ...
        const texture = new GLTexture(gl, textureID, null, null, internalformat, format, type);
