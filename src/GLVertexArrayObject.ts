...
import { GLAttribute } from './shader/GLAttribute';
...
export class GLVertexArrayObject {
    ...
    protected activate() {
        ...
        for (let i = 0, attribute: Attribute; (attribute = this.attributes[i]); i++) {
    ...
    public static setVertexAttribArrays(gl: WebGL2RenderingContext, attributes: Array<Attribute>, unbind = false): void {
        for (let i = 0, attribute: Attribute; (attribute = attributes[i]); i++) {
            GLVertexArrayObject.setVertexAttribArray(gl, attribute, unbind);
        }
    }

    public static setVertexAttribArray(gl: WebGL2RenderingContext, attribute: Attribute, unbind = false): void {
